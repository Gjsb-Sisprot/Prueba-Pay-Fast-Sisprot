...
function Calendar({
  ...
  // By default, disable dates after today unless the caller explicitly
  // provides `disabled` (recommended) or `toDate` (legacy). This ensures
  // calendars across the app cannot select future dates by accident.
  // Narrow the props type instead of using `any` to satisfy ESLint/TS rules
  const p = props as Partial<React.ComponentProps<typeof DayPicker>>
  const callerDisabled = p.disabled
  // read legacy `toDate` from props without using `any`
  const callerToDate = (props as Record<string, unknown>)['toDate'] as Date | undefined

  const effectiveDisabled = callerDisabled ?? (callerToDate ? { after: callerToDate } : { after: new Date() })

  // Avoid forwarding deprecated `toDate` to DayPicker; prefer `disabled`.
  // Use Record<string, unknown> to manipulate unknown keys without `any`.
  const restRecord: Record<string, unknown> = { ...(props as Record<string, unknown>) }
  if ('toDate' in restRecord) {
    // remove legacy prop if present
    delete restRecord.toDate
  }
  const effectiveProps = {
    ...(restRecord as Partial<React.ComponentProps<typeof DayPicker>>),
  ...
  return (
    <DayPicker
      ...
      {...(effectiveProps as React.ComponentProps<typeof DayPicker>)}
    />
  )
}
